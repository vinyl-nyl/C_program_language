...
void disp_car(int car_number, int distance){ // 함수 disp_car 선언 및 지정(반환값이 없고, 매개변수는 2개)
    int i; // 변수 i 선언
...
int main(){
    int i; // 변수 i 선언
    int car1_dist = 0, car2_dist = 0; // 경주에 참여하는 자동차 변수 선언 및 초기값 지정

    srand((unsigned)time(NULL)); //

    for(i = 0; i < 6; i++){ // 변수 i가 0부터 5까지 1씩 증가하는 반복문
